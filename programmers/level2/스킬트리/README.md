...
- 선행 스킬이라는 파라미터는 반드시 처음 스킬부터 순서대로 배워야 하는 것이기 때문에 우선 스킬트리라는 파라미터값을 선행 스킬에 존재하는 값만 추리도록 작성하였습니다.

```python
filtered_skills = [s for s in tree if s in skill]
```

- 추려진 스킬트리가 선행 스킬의 내용과 동일 길이 내에서 값이 같을 경우에만 반환값을 증가시키도록 작성하였습니다.
